...
void launchpad::send_hex(const string& hex)
{
	...
	auto m = MidiMessage::createSysExMessage(&k[0], (int)k.size());
	send_message_now(m);
}
...
void launchpad::send_LED()
{
	...
	//printHex(p - 5, 5);
	auto m = MidiMessage::createSysExMessage(LED_raw_data.get(), p - LED_raw_data.get());
...
vector<uint8_t> hexStringToBytes(const string& input)
{
	...
	for (size_t i = 0; i < input.size(); i++)
	{
		if (isxdigit(input[i]))
		{
			...
			uint8_t byte = (uint8_t)stoi(input.substr(i, 2), nullptr, 16);
...
map<string, tuple<string, uint8_t>> const launchpad::VPID_map = {
